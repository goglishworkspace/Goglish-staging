...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase
    ...
    .select()
    .maybeSingle();
  if (error) return apiError("تعذر تحديث الحدث", null, 400);
  if (!data) return apiError("الحدث غير موجود", null, 404);
  return apiSuccess(data, "تم تحديث الحدث");
}
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error, count } = await supabase
    ...
    .delete({ count: "exact" })
    .eq("id", id);
  if (error) return apiError("تعذر حذف الحدث", null, 400);
  if (!count) return apiError("الحدث غير موجود", null, 404);
  return apiSuccess(null, "تم حذف الحدث");
}
