...
function AddChildForm() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    linkChild.mutate(nationalId.trim(), {
      onSuccess: (data) => {
        toast.success(
  ...
  return (
    <form onSubmit={onSubmit} className="flex w-full flex-wrap items-end gap-2">
      <div className="flex min-w-0 flex-1 flex-col gap-1.5">
        <label htmlFor="child-national-id" className="text-caption text-muted-foreground">
          الرقم القومي للطالب
        </label>
        <Input
      ...
      <Button type="submit" disabled={linkChild.isPending || nationalId.trim().length === 0}>
        <UserPlus />
        إضافة ابن/ابنة
      </Button>
    </form>
