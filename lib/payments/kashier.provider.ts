  ...
  async createCheckout(params: CreateCheckoutParams): Promise<CreateCheckoutResult> {
    ...
    const response = await fetch(`${apiBaseUrl(mode)}/v3/payment/sessions`, {
      ...
      body: JSON.stringify({
        ...
        merchantId,
        // NOT sent in the body: the live API started hard-rejecting it with
        // `"mode" is not allowed` (2026-08-22), on top of the customer-field
        // issue above - test vs live is already fully determined by which
        // host we're hitting (apiBaseUrl(mode) above), so this was always
        // redundant, just newly rejected outright instead of ignored.
