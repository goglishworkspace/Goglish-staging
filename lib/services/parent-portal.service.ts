...
export async function linkChildToParent(parentUserId: string, childIdentifier: string): Promise<LinkChildResult> {
  const identifier = childIdentifier.trim();
  if (!identifier) return { ok: false, reason: "invalid_format" };
  ...
  } else if (/^\d{14}$/.test(identifier)) {
    // 2. Fallback to national ID decryption if 14 digits
